...
setup_zoom = function(graph, toggleFlag){
  var  svg = graph.svgForPCA;
  var options = graph.options;
  var page_options = graph.page_options;
         ...
         .on("brush", function(){
           ...
           svg.selectAll(".dot")
             .classed("selected", function(d) {
               var extent = brush.extent();
               var isBrushed;
               isBrushed = (extent[0][0] <= d[options.xDomain] && d[options.xDomain] < extent[1][0] &&
  								 extent[0][1] <= d[options.yDomain] && d[options.yDomain] < extent[1][1]);
               return isBrushed;
             });
         })
...
setup_brush = function(graph){
  ...
  setup_graph = function(graph){
    ...
    */
    ...
    if (graph.toggleFlag) {
      graph = setup_zoom(graph, true);
    } else {
      graph = setup_zoom(graph, false);
    }
    //graph = setup_zoom(graph,false);
