...
//setup margins
  setup_margins = function(graph){
    ...
    page_options.barChartHeight = options.barChartHeight;
    page_options.barChartWidth = options.barChartWidth;
...
function zoom_end(){
  ...
  setup_y_axis_for_bar_chart = function(graph){
    ...
    var barChartScaleY = d3.scale.linear()
        .range([0, page_options.barChartHeight]);
    //setup domain
  ...
  setup_bar = function(graph){
    ...
    .enter().append("rect")
      ...
      .attr("height", function(d) {return options.barChartHeight - graph.barChartScaleY(d.eigenvalue); })
  ...
  setup_svg = function(graph){
    ...
    var scatterWidth = page_options.width;
    var scatterHeight = page_options.height;
    ...
    var svg2 = d3.select(options.target).append("svg")
        ...
        .attr("height",page_options.barChartHeight)
      .append("g")
        // this is just to move the picture down to the right margin length
        .attr("transform", "translate(" + left + "," + top + ")");
