...
//setup margins
  ...
  setup_x_axis = function(graph){
    ...
    //setup domain
    scaleX.domain(d3.extent(options.data, function(d){return d.component1;})).nice();
    //setup xaxis
  ...
  setup_y_axis = function(graph){
    ...
    //setup domain
    scaleY.domain(d3.extent(options.data, function(d){return d.component2;})).nice();
    //setup y axis
  ...
  setup_scatter = function(graph){
    ...
    if(options.domain != undefined){
      page_options.domain = options.domain
      page_options.color = d3.scale.ordinal().domain(page_options.domain).range(options.domain_colors);
    } else {
      page_options.color = d3.scale.category10();
    }

    var domain = page_options.domain;
    ...
    svg.selectAll(".dot")
      ...
      .enter()
        ...
        .attr("cx", function(d){return graph.scaleX(d.component1);})
        .attr("cy", function(d){return graph.scaleY(d.component2);})
        .style("fill", function(d) {
          var colorOption;
          if (domain === options.metaData.option1) {
            colorOption = d.option1;
          }
          if (domain === options.metaData.option2) {
            colorOption = d.option2;
          }
          if (domain === options.metaData.option3) {
            colorOption = d.option3;
          }
